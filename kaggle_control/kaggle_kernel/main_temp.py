...
WARN FFMPEG TIME CONTRACT — STRICT REQUIREMENTS:
...
def download_youtube_video(url, output_dir="."):
    ...
    """
    print(f"[SCAN] Debug: yt-dlp version: {yt_dlp.version.__version__}")
    ...
    if cookies_env:
        ...
        try:
            ...
            print(f"WARN Failed to write cookies file: {e}")
            cookies_path = None
    elif os.path.exists(cookies_path):
        print("🍪 Found existing cookies.txt file locally.")
    else:
        cookies_path = None
        print("WARN No YOUTUBE_COOKIES env var and no cookies.txt found.")
...
def transcribe_video(video_path):
    ...
    if torch.cuda.is_available():
        ...
        print(f"[MIC] Transcribing with Faster-Whisper GPU ({torch.cuda.get_device_name(0)})")
        print(f" Model: {model_size} | Device: {device} | Compute: {compute_type}")
    else:
        device = "cpu"
        compute_type = "int8"
        model_size = "base"
        print(f"[MIC] Transcribing with Faster-Whisper CPU (model: {model_size})")
    ...
    if device == "cuda":
        for ctype in ["float16", "int8", "float32"]:
            try:
                ...
                print(f"OK Using compute_type: {ctype}")
                break
            except ValueError as e:
                print(f"WARN {ctype} no soportado, probando siguiente...")
                if model is None:
...
def get_viral_clips(transcript_result, video_duration):
    ...
    try:
        ...
        try:
            ...
            print(f"WARN Could not calculate cost: {e}")
            cost_analysis = None
        # ------------------------
...
if __name__ == '__main__':
    ...
    if args.skip_analysis:
        ...
        # 3. Transcribe
        print("[MIC] Starting transcription...")
