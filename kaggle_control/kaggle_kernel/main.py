...
def check_and_install_dependencies():
    """Verifica e instala las dependencias necesarias en Kaggle"""
    required_packages = [
        'scenedetect', 'mediapipe', 'ultralytics', 'faster-whisper',
        'opencv-python', 'yt-dlp', 'torch', 'google-genai', 'python-dotenv'
    ]
    ...
    if missing:
        print(f"⚠️ Instalando dependencias: {', '.join(missing)}")
...
# Fix for OpenMP Error #15 (multiple libiomp5md.dll loaded by PyTorch/NumPy)
...
def download_youtube_video(url, output_dir="."):
    ...
    if cookies_env:
        print("🍪 Found YOUTUBE_COOKIES env var, creating cookies file locally...")
        try:
        ...
        print("🍪 Found existing cookies.txt file locally.")
    else:
        cookies_path = None
        print("⚠️ No YOUTUBE_COOKIES env var and no cookies.txt found.")
...
Technical Details: {str(e)}
    ...
    try:
        ...
        import sys
        import traceback
        print(f"🚨 YOUTUBE DOWNLOAD ERROR (ydl.download) 🚨", file=sys.stderr)
...
def process_video_to_vertical(input_video, final_output_video):
    ...
    command = [
        ...
        '-r', str(fps), '-i', '-', '-c:v', vcodec,
        *encode_opts, '-an', temp_video_output
    ]
...
def transcribe_video(video_path):
    ...
    if torch.cuda.is_available():
        device = "cuda"
        model_size = "small"  # Cambiado de large-v3 a small para mayor velocidad
    ...
    # Intentar crear el modelo con diferentes compute_type
    model = None
    compute_type_used = None
    if device == "cuda":
        ...
        # Si todos fallan, forzar float32 como último recurso
        if model is None:
            print("⚠️ Ningún compute_type estándar funcionó, forzando float32...")
            model = WhisperModel(model_size, device=device, compute_type="float32")
            compute_type_used = "float32"
    else:
...
def find_kaggle_input_video():
    """Busca automáticamente un video en /kaggle/input/ de forma recursiva."""
    import glob
    video_extensions = ['*.mp4', '*.avi', '*.mov', '*.mkv', '*.webm']
    
    # Búsqueda recursiva en todo /kaggle/input/
    for ext in video_extensions:
        # Buscar en todo el árbol recursivamente
        pattern = f'/kaggle/input/**/{ext}'
        videos = glob.glob(pattern, recursive=True)
        if videos:
            return videos[0]
    
    # Si no encuentra con glob, usar os.walk como fallback
...
if __name__ == '__main__':
    ...
    # Si no se proporcionó input o URL, buscar automáticamente en /kaggle/input/
    if not args.input and not args.url:
        kaggle_video = find_kaggle_input_video()
        if kaggle_video:
            print(f"🔍 Video encontrado en Kaggle: {kaggle_video}")
            args.input = kaggle_video
        else:
            print("❌ No se encontró ningún video en /kaggle/input/ y no se proporcionó -i o -u")
            sys.exit(1)
    ...
    if args.skip_analysis:
        ...
        if hasattr(args, 'job_id') and hasattr(args, 'backend_url'):
            update_progress(args.job_id, args.backend_url, 'transcribing', 'Transcribing audio...', 10)
    ...
    print("🤖 Analyzing with Gemini...")
    if hasattr(args, 'job_id') and hasattr(args, 'backend_url'):
        update_progress(args.job_id, args.backend_url, 'analyzing', 'AI analysis in progress...', 30)
    ...
    if not clips_data or 'shorts' not in clips_data:
        ...
        for i, clip in enumerate(clips_data['shorts']):
            ...
            if hasattr(args, 'job_id') and hasattr(args, 'backend_url'):
                clip_progress = 50 + int(40 * (i / total_clips))
                update_progress(args.job_id, args.backend_url, 'clipping', f'Processing clip {i+1}/{total_clips}', clip_progress, f'clip_{i+1}.mp4', total_clips, i)
    ...
    # ========================================================================
    # UPLOAD RESULTS AS KAGGLE DATASET (para poder descargar después)
    # ========================================================================
    print("\n📤 Subiendo resultados como dataset de Kaggle...")
    try:
        ...
        dataset_metadata = {
            'title': f'OpenShorts Output - {video_title}',
            'id': f'joelowtok/openshorts-output-{int(time.time())}',
        ...
        print(f"✅ Dataset creado con los clips procesados")
    except Exception as e:
        print(f"⚠️  No se pudo subir el dataset: {e}")
        print(f"   Los archivos están disponibles en: {output_dir}/")
